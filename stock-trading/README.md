# 📈 美股量化交易系统

基于 Massive.com API + 舆情分析 + LLM 自主决策的完整量化交易系统，内置自动回测和策略迭代机制。

## 🎯 功能特性

- **数据获取**: 通过 Massive API 获取美股价格、技术指标、公司行为等数据
- **舆情分析**: 整合 Finviz 新闻、Reddit 社交、Seeking Alpha 分析师评级
- **LLM 决策**: 基于综合数据生成交易决策 (买入/卖出/持有 + 具体股数)
- **自动回测**: 基于历史数据验证策略效果
- **策略迭代**: 自动复盘→调整→再验证，直到达到收益目标

## 📁 项目结构

```
stock-trading/
├── main.py                    # 主入口脚本
├── skill.yaml                 # OpenClaw Skill 配置
├── requirements.txt           # Python 依赖
├── skills/
│   └── SKILL.md              # 技能文档
├── src/
│   ├── __init__.py
│   ├── config.py             # 配置文件
│   ├── massive_api.py        # Massive API 封装
│   ├── sentiment_api.py      # 舆情数据获取
│   ├── llm_decision.py       # LLM 决策模块
│   ├── backtest.py           # 回测系统
│   └── strategy_runner.py    # 策略迭代
├── strategies/
│   ├── __init__.py
│   └── default_strategy.py   # 默认策略模板
├── data/                     # 回测结果存储
└── logs/                     # 日志文件
```

## 🚀 快速开始

### 1. 安装依赖

```bash
cd stock-trading
python3 -m pip install -r requirements.txt
```

### 2. 分析股票

```bash
python3 main.py analyze AAPL
```

输出示例:
```
📊 分析股票：AAPL

⏳ 获取市场数据...
   当前价格：$185.50
   今日开盘：$184.00
   ...

⏳ 计算技术指标...
   RSI(14): 45.2
   MACD: 2.02
   ...

⏳ 分析舆情...
   综合评分：0.58 (Positive)
   ...

🤖 生成 LLM 决策提示词...
```

### 3. 运行回测

```bash
python3 main.py backtest AAPL --start 2024-01-01 --end 2024-12-31
```

输出示例:
```
📊 回测结果 - AAPL
============================================================
回测周期：2024-01-01 至 2024-12-31 (252 交易日)

💰 资金变化:
  初始资金：$10,000.00
  最终资金：$12,350.00
  总收益：  $2,350.00

📈 绩效指标:
  总收益率：  +23.50%
  年化收益：  +23.50%
  最大回撤：  -8.25%
  夏普比率：  1.85
  胜率：      62.5%
  盈亏比：    2.1
...
```

### 4. 策略迭代

```bash
python3 main.py iterate AAPL,MSFT,GOOGL,NVDA --start 2024-01-01 --end 2024-12-31 --target-return 25
```

系统会自动:
1. 对每只股票进行回测
2. 检查是否达到目标 (收益率≥25%, 回撤≤-15%, 夏普≥1.5)
3. 未达标则分析原因并提供调整建议
4. 继续测试下一只股票
5. 生成总结报告

### 5. 模拟交易

```bash
# 执行今日模拟交易
python3 main.py paper GOOGL,META,AAPL --capital 10000 --strategy optimized_v2

# 显示绩效报告
python3 main.py paper GOOGL,META,AAPL --show-report

# 导出报告
python3 main.py paper GOOGL,META,AAPL --export data/paper_report.json
```

系统会:
1. 获取实时市场数据
2. 执行策略决策
3. 记录交易到数据库 (data/trading.db)
4. 更新持仓和每日快照
5. 生成交易报告

### 6. 检查市场状态

```bash
python3 main.py status
```

## 📊 策略配置

编辑 `strategies/default_strategy.py` 自定义交易逻辑:

```python
def default_strategy(row, indicators):
    buy_conditions = []
    sell_conditions = []
    
    # RSI 超卖买入
    if indicators.get('rsi_14', 50) < 30:
        buy_conditions.append("RSI 超卖")
    
    # MACD 金叉买入
    if indicators.get('macd', 0) > indicators.get('macd_signal', 0):
        buy_conditions.append("MACD 金叉")
    
    # 至少 2 个条件满足时买入
    if len(buy_conditions) >= 2:
        return 'buy'
    
    return 'hold'
```

## 🎯 目标配置

编辑 `src/config.py` 调整收益目标:

```python
TARGET_METRICS = {
    "min_total_return": 20.0,      # 最低总收益率 20%
    "max_drawdown": -15.0,         # 最大回撤不超过 -15%
    "min_sharpe_ratio": 1.5,       # 最低夏普比率 1.5
    "min_win_rate": 55.0,          # 最低胜率 55%
    "min_trades": 20               # 最少交易次数
}
```

## 🔧 OpenClaw 集成

在 OpenClaw 中使用:

```
/stock-trading analyze AAPL
/stock-trading backtest MSFT --start 2024-01-01
```

或通过 LLM 直接获取交易决策:

```
请分析 AAPL 的当前情况，给出交易建议
```

系统会自动:
1. 获取最新市场数据
2. 计算技术指标
3. 分析舆情情绪
4. 调用 LLM 生成决策
5. 返回具体的交易建议 (买入/卖出/持有 + 股数)

## ⚠️ 注意事项

### API 限制 (Massive Starter)
- 数据延迟：15 分钟
- 历史数据：最多 5 年
- API 调用：无限
- Trades/Quotes 数据：不可用

### 交易风险
- 本系统仅供学习和研究
- 回测结果不代表未来收益
- 实盘交易需谨慎，建议先用模拟盘验证
- 日级别交易不适合高频操作

## 📝 输出文件

回测结果保存在 `data/` 目录:

- `backtest_{SYMBOL}_{START}_{END}.json` - 单次回测结果
- `iteration_results.json` - 策略迭代总结

## 📄 License

MIT License

---

**Version**: 4.0.0
**Author**: 小 X (for GX)
**Updated**: 2026-02-27
