...
def generate_mock_data(symbol: str, start_date: str, end_date: str) -> pd.DataFrame:
    ...
    # 趋势和波动
    trend = np.random.uniform(-0.1, 0.2)  # 年化趋势 -10% 到 +20%
    volatility = np.random.uniform(0.15, 0.35)  # 年化波动率 15%-35%
...
class StrictBacktestEngine:
    ...
    def run_single_stock(self, symbol: str, start_date: str, end_date: str) -> dict:
        """单只股票回测"""
        print(f"   📈 回测 {symbol}...")
...
def run_massive_backtest(symbols: list, start_date: str, end_date: str):
    """批量回测"""
    print(f"\n{'='*80}")
    print(f"🚀 大规模美股回测启动 (演示模式)")
    print(f"{'='*80}")
    print(f"标的数量: {len(symbols)} 只")
    print(f"回测周期: {start_date} ~ {end_date}")
    print(f"策略: 双均线Crossover + RSI过滤")
    print(f"初始资金: $10,000/只股票")
    print(f"{'='*80}\n")
    
    engine = StrictBacktestEngine(initial_capital=10000)
    results = []
    ...
    for i, symbol in enumerate(symbols):
        ...
        if (i + 1) % 10 == 0 or (i + 1) == len(symbols):
            print(f"   进度: {i+1}/{len(symbols)} ({len(results)}成功)")
...
def print_report(report: dict):
    ...
    print("\n" + "="*80)
    print("✅ 关键说明:")
    print("   • 本演示使用模拟数据验证策略逻辑")
    print("   • 严格避免未来函数：每天只用当日及之前数据")
    print("   • 信号在盘前生成，交易在开盘价执行")
    print("   • 实际运行需接入真实数据源(yfinance/polygon)")
    print("="*80)
...
def save_report(report: dict):
    ...
    filename = f"demo_backtest_100stocks_{timestamp}.json"
